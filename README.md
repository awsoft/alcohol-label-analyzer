...
**Tools used.** The app was scaffolded in **Google AI Studio** (initial React + Gemini starter and early prompt experiments), iterated on with **Claude Sonnet 4.5** during early feature development, and then built out with **Claude Fable 5** via Claude Code — covering the serverless key architecture, the structured-output refactor, the application-verification and batch modes, the TTB placement-rule research (27 CFR parts 4, 5, 7, and 16), model benchmarking, deployment debugging, and documentation. At runtime the app calls Google **Gemini** (`gemini-3.5-flash` for the deep analysis modes; `gemini-3.1-flash-lite` for sub-5-second verification), and the example label photos were shot on an iPhone.
